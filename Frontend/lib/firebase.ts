...
// Enable offline persistence on web where available.
if (Platform.OS === 'web') {
  enableIndexedDbPersistence(db).catch((err) => {
    // If multiple tabs open or unsupported, fall back to default (network-only)
    // We silently ignore to avoid breaking the app at runtime.
